#!/usr/bin/env python3
"""
Eduardo García — terminal profile promo.

Renders `eduardo-terminal.gif`: a ~36s looping sequence that opens as a terminal,
...
OUT = os.path.join(HERE, "eduardo-terminal.gif")
OUT_WEBP = os.path.join(HERE, "eduardo-terminal.webp")
...
if os.name == "nt":
    ...
    MONO_BOLD_PATH = r"C:\Windows\Fonts\consolab.ttf"
    DISP_PATH = r"C:\Windows\Fonts\arialbd.ttf"
    DISP_REG_PATH = r"C:\Windows\Fonts\arial.ttf"
else:
...
BT_NODES = {
    "root":    (440.0, 100.0, "\u2192", "product build"),
    "guard":   (270.0, 172.0, "?", "validate"),
    "run":     (610.0, 172.0, "\u21c9", "implement"),
    "battery": (185.0, 244.0, "\u25cb", "requirements ok"),
    "rtl":     (355.0, 244.0, "\u25b8", "refine scope"),
    "track":   (525.0, 244.0, "\u25b8", "build api"),
    "stream":  (695.0, 244.0, "\u25b8", "build ui"),
}
...
GRAPH_NODES = [
    ("orchestrator", 236.0, 140.0),
    ("retrieval", 158.0, 258.0),
    ("database", 348.0, 318.0),
    ("llm", 528.0, 132.0),
    ("tools", 712.0, 232.0),
    ("user", 588.0, 330.0),
]
...
def status_bar(d, k, label):
    ...
    text(d, W - 28, H - 24, "github.com/eduardogarcia270402", F_MONO_XS, fade(FAINT, k), anchor="ra")
...
TERM_LINES = [
    ...
    (0,  0, [("eduardo", MUTED), (" on ", FAINT), ("main", MUTED),
             (" [!?] ", FAINT), ("took 16s", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("ssh eduardo@github.com", BODY)], True),
    (17, 1, [("handshake ", FAINT), ("·" * 14, LINE), (" ok", BODY)], False),
    (20, 1, [("locale ", FAINT), ("·" * 17, LINE),
             (" quito, ecuador · utc-05", BODY)], False),
...
JOKE_LINES = [
    (0,  0, [("eduardo", MUTED), (" on ", FAINT), ("main", MUTED),
             (" [!?]", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("stack inspect --all", BODY)], True),
    (15, 1, [("web · mobile · cloud · ai", BODY)], False),
    (18, 1, [("keep learning. keep building.", MUTED)], False),
]
...
SPEC = [
    ...
    ("based", "quito, ecuador"),
    ("building", "full-stack · mobile · automation"),
    ("also", "ai agents · linux · cloud"),
]

TRAJECTORY = [
    (2022.2, "engineering", "software"),
    (2022.9, "frontend", "angular"),
    (2023.7, "backend", "node · python"),
    (2024.6, "mobile", "flutter"),
    (2025.4, "cloud", "aws · docker"),
    (2026.2, "agentic ai", "rag · n8n"),
]
TRAJ_T0, TRAJ_T1 = 2021.8, 2026.8
...
def scene_identity(d, f):
    ...
    if t >= 1:
        text(d, x0, 78, "Eduardo García", F_DISP_XL, HEADING)
    ...
    if t >= 5:
        n = int((t - 5) * 3.4)
        l1 = "Web, mobile, cloud, and AI."
        l2 = "Useful software, built to scale."
...
def scene_bt(d, f):
    ...
    if t >= 1:
        ...
        n = int((t - 1) * 1.9)
        text(d, 54 + 2 * CW, 52, "bt tick --product build"[:n], F_MONO, BODY)
    ...
    if t >= 32:
        ...
        ctext(d, W / 2, 312, "requirements become working software.",
              F_MONO_S, fade(MUTED, a))
    if t >= 34:
        a = ease_out(seg(t, 34, 4))
        ctext(d, W / 2, 334, "frontend \u00b7 backend \u00b7 data \u00b7 delivery",
...
def scene_graph(d, f, positions, alive):
    ...
    if t >= 1:
        text(d, 54, 52, "\u2192 ", F_MONO, fade(ACCENT, 0.9))
        n = int((t - 1) * 1.9)
        text(d, 54 + 2 * CW, 52, "orchestrate ai agents"[:n], F_MONO, BODY)

    draw_graph(d, t, ease_out(seg(t, 5, 6)), positions, alive)

    if t >= 24:
        a = ease_out(seg(t, 24, 5))
        ctext(d, W / 2, 314, "context in. useful answers out.",
              F_MONO_S, fade(MUTED, a))
    if t >= 27:
        a = ease_out(seg(t, 27, 4))
        ctext(d, W / 2, 336,
              "rag \u00b7 llms \u00b7 semantic search \u00b7 automation",
              F_MONO_XS, fade(FAINT, a * 0.9))
...
def scene_trajectory(d, f):
    ...
    n = int(max(0, t) * 2.4)
    text(d, 54 + 2 * CW, 54, "learning --since 2022"[:n], F_MONO, BODY)
    ...
    if t >= 20:
        a = ease_out(seg(t, 20, 6))
        ctext(d, W / 2, H - 66,
              "software engineering · continuous learning · practical building",
...
def scene_sign(d, f):
    ...
    if t >= 3:
        ...
        ctext(d, cx, 206, "eduardo", F_MONO_XL, fade(HEADING, b))

    if t >= 5:
        b = ease_out(seg(t, 5, 4))
        ctext(d, cx, 256, "Eduardo García — software engineering student",
    ...
    if t >= 9:
        ...
        left = "github.com/EduardoGarcia270402"
        right = "linkedin.com/in/eduardogarcia270402"
        gap = 4.0
    ...
    if t >= 11 and (f // 4) % 2 == 0:
        half = text_w("eduardo", F_MONO_XL) / 2.0
        cwx = cw(F_MONO_XL)
        x = cx + half + cwx * 0.35
